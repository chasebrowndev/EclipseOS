...
6. **Effects parsed but not drawn**: `decoration` and `animations` now parse
   ...
   own shadow (also verified nested under winit). Still not drawn: `blur`
   (needs multi-pass framebuffers) and animation
...
5. **Effects (9b)** — optional by COMP-16 Open Decision 1. Borders, opacity,
   dim-inactive, rounding and shadows are in; "no animation, no blur" is
