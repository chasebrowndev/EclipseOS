...
| 8 | Screen sharing via xdg-desktop-portal | **partial** | `zwlr_screencopy_v1` behind a fail-closed capture gate + frame redaction (ADR 0027) is done and measured: 851 frames in ~11 s to a direct client, and a `redact-app-id` window comes back entirely black while the same scene without the entry does not. `xdg-desktop-portal-wlr` allowlisted by the user (ADR 0029) reaches PipeWire but stalls after one frame in xdpw 0.8.3, whose wlr path restarts the loop only from its PipeWire `on_process`; the gate ("a video call with screen share") is therefore not met. Also outstanding: the COMP-10 consent prompt (M14) — today the allowlist entry is the only gate, and it cannot tell portal clients apart — and lock-denies-capture is unit-tested only, since no locker available here will lock a nested session |
