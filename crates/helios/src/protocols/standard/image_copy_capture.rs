...
/// mutable half is owned by `HeliosState` and reachable only from the single
...
impl ImageCopyCaptureState {
    ...
    pub fn new(dh: &DisplayHandle, allow: Allowlist) -> Self {
        dh.create_global::<HeliosState, ExtOutputImageCaptureSourceManagerV1, _>(
            VERSION,
            ManagerData {
                dh: dh.clone(),
                allow: allow.clone(),
        ...
        dh.create_global::<HeliosState, ExtImageCopyCaptureManagerV1, _>(
            VERSION,
            ManagerData {
                dh: dh.clone(),
                allow,
...
impl GlobalDispatch<ExtOutputImageCaptureSourceManagerV1, ManagerData> for HeliosState {
    fn bind(
        _state: &mut Self,
        _dh: &DisplayHandle,
        _client: &Client,
...
impl Dispatch<ExtOutputImageCaptureSourceManagerV1, ()> for HeliosState {
    fn request(
        state: &mut Self,
        client: &Client,
...
impl Dispatch<ExtImageCaptureSourceV1, SourceData> for HeliosState {
    fn request(
        _state: &mut Self,
        _client: &Client,
...
impl GlobalDispatch<ExtImageCopyCaptureManagerV1, ManagerData> for HeliosState {
    fn bind(
        _state: &mut Self,
        _dh: &DisplayHandle,
        _client: &Client,
...
impl Dispatch<ExtImageCopyCaptureManagerV1, ()> for HeliosState {
    fn request(
        state: &mut Self,
        client: &Client,
...
impl Dispatch<ExtImageCopyCaptureSessionV1, SessionData> for HeliosState {
    fn request(
        state: &mut Self,
        _client: &Client,
...
impl Dispatch<ExtImageCopyCaptureFrameV1, FrameData> for HeliosState {
    fn request(
        state: &mut Self,
        _client: &Client,
...
fn capture_request(state: &mut HeliosState, resource: &ExtImageCopyCaptureFrameV1, fid: u64) {
    let Some(frame) = state.image_copy.frames.get_mut(&fid) else {
        resource.failed(FailureReason::Stopped);
        return;
    };
    ...
    // never depend on damage that may not come (a nested helios whose host
...
fn output_size(state: &HeliosState, output_id: u64) -> Option<(i32, i32)> {
