...
//! display the ability to see and poke every other. helios does not widen
...
fn x11_has_focus(state: &HeliosState) -> bool {
    state.focus.as_ref().is_some_and(|w| w.x11_surface().is_some())
}

fn window_for(state: &HeliosState, surface: &X11Surface) -> Option<Window> {
    state
        .space
        .elements()
...
impl XwmHandler for HeliosState {
