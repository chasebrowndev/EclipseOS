...
//!
//! A line-delimited JSON-RPC 2.0 server on `$XDG_RUNTIME_DIR/eclipse/helios.sock`,
...
//! display. Connection state lives in [`IpcState`] inside `HeliosState`,
...
/// All control-socket state. Owned by `HeliosState`.
#[derive(Default)]
pub struct IpcState {
...
pub fn start(state: &mut HeliosState, handle: &LoopHandle<'static, HeliosState>) {
    ...
    let path = dir.join("helios.sock");
    if path.exists() {
        // A socket that still answers belongs to a live compositor. Never
        // steal it; a second helios simply runs without a control socket.
...
pub fn cleanup(state: &HeliosState) {
...
fn accept(state: &mut HeliosState, stream: UnixStream) {
...
fn readable(state: &mut HeliosState, id: u64, stream: &UnixStream) -> PostAction {
...
/// input).
fn handle_line(state: &mut HeliosState, conn: u64, line: &[u8]) -> Option<String> {
...
pub fn emit(state: &mut HeliosState, kind: &str, params: Value) {
