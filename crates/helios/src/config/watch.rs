...
/// COMP-13 §1.2. Long enough to absorb a write-and-rename or a `helios.d`
...
fn watch_dirs(state: &HeliosState) -> Vec<PathBuf> {
    ...
    if let Some(base) = dirs_config() {
        ...
        push(base.join("eclipse/helios.d"));
    }
...
/// simpler and correct for `helios.d` (COMP-13 §1.2 "never half-apply").
...
fn schedule(state: &mut HeliosState) {
...
pub fn reload_now(state: &mut HeliosState) {
