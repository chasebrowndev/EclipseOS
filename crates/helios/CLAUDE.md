...
- One thread, one `HeliosState`, one calloop loop. No `Arc<Mutex<..>>` around state.
...
- Logs go to journald: `journalctl --user -t helios -o cat --since "5 min ago"`.
- Nested test under Hyprland: `WAYLAND_DISPLAY=wayland-1 ./target/debug/helios --backend winit`,
