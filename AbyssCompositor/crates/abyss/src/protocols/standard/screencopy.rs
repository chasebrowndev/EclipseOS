...
use crate::{config::Allowlist, render::capture, state::AbyssState};
...
impl GlobalDispatch<ZwlrScreencopyManagerV1, ManagerData> for AbyssState {
    fn bind(
        _state: &mut Self,
        _dh: &DisplayHandle,
        _client: &Client,
...
impl Dispatch<ZwlrScreencopyManagerV1, ()> for AbyssState {
    fn request(
        state: &mut Self,
        client: &Client,
...
impl Dispatch<ZwlrScreencopyFrameV1, FrameData> for AbyssState {
    fn request(
        state: &mut Self,
        _client: &Client,
