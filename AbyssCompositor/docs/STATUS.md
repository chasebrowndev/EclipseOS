...
|---|---|---|
| `wlcs` headless conformance | COMP-15 §1 | harness and the blocking `conformance` job exist; the suite runs green off-CI (555 passed / 319 skipped / 0 failed at `ulimit -n 1024`; of the skips only 42 are real gaps — foreign-toplevel and virtual-pointer — the rest being dead protocols, wlcs self-tests and the touch harness gap); awaiting a green run on CI’s llvmpipe runner |
