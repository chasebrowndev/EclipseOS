...
redaction, window-rules engine, benchmark harness); all four now have code —
9c and 9d have landed — 9c's `wlcs` gate is now green in CI, which closes it —
9e is partial, and 9f's harness exists with no subjects to point it at until
Phase 2.
...
blocked on hardware, or is 9e's two missing fields and 9f's benchmark subjects;
9b (effects) is done and 9c is done, its `wlcs` suite green on CI as well as
off it.
...
actually taken, COMP-14 budgets), milestone 6's gate (suspend/resume, lid), the
rest of 9e, and the benchmark subjects 9f is waiting on.

**Open vs. not, as of 2026-09-10.** Closed and needing nothing further:
milestones 1, 2, 5, 7, 9, 9a, 9b, 9c; spec gaps 1, 2, 3, 4 and 5. Partly hardware-verified, with the rest of the
...
9's bar gate, what COMP-16 v0.2's Phase 1 additions still owe (the rest of 9e
and 9f's benchmark subjects, which cannot exist before the milestones that
create them), and the whole of Phase 2 (milestones 10–25), whose
...
| COMP-05 shell | `crates/abyss/src/shell/mod.rs` | radiant (default, weighted n-ary tree, drop-zone drag, priority; ADR 0058) + dwindle classic + master, workspaces, floating, focus. App identity is a `/proc` stopgap (`data_control.rs:14` TODO) — the provenance record is Phase 2. Window rules match at map time and re-evaluate on commit (`shell/rules.rs`); see milestone 9e for what is still missing. |
...
17. **No semantic-tree source** — `protocols/semantic/` does not exist, so
    nothing ever supplies the node tree COMP-02 §7 redacts against. This is
    the remaining half of milestone 9d: the redaction machinery and its
...
1. **Boot it from the greeter.** The compositor starts on real KMS and paints
   ...
   **Superseded 2026-09-19: the Framework install logged in to Abyss from
   ...
   stays here. As of 2026-09-18 it has not been done.
...
11. **COMP-14 gained five budgets** (A-14) covering `classify()`, the
    ...
    They join the existing §2 targets in never having been measured, but
    the harness now exists: milestone 9f landed `bench/` with all fourteen
    §2.1/§2.1b budgets encoded as data (ADR 0043). What is missing is
    subjects — `classify()`, the irreversible matcher, provenance and the
    egress proxy are all Phase 2 code — so milestone 4's exit gate stays
    unclosable as written, as it has been since v0.1.
...
**The first distribution packages actually build.** Landed with PR #19
(`ce2e86f`) and fixed into working order by PR #20 (`2e7824a`): one `makepkg`
run over one source build produces `eclipseos-abyss`, `eclipseos-desktop`,
`eclipseos-policyd` and `eclipseos-meta`, and `namcap` reports no errors against
them. That is F-07 §7 — the packaging the document has described since D-01 now
exists as artefacts rather than as a plan. What it is *not* is evidence that
anything installs or runs from those packages: nobody has installed them onto a
machine and booted the result. A clean build and a clean `namcap` are the floor,
not the gate.
