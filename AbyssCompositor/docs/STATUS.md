...
| 9d | Node-level redaction, fail-closed on stale/absent tree | **in progress** (branch `comp02-m09d-node-redaction`) | `render/capture.rs` is surface-level only today. The redaction machinery is being built now on that branch: region-based redaction, the fail-closed staleness resolver, and tests driven from a synthetic tree. COMP-02 §7 defines the absent-tree case, so that half needs no semantic tree; the outstanding half is the tree *source* — COMP-09, `protocols/semantic/`, which does not exist (milestone 22). |
