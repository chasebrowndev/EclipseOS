...
|---|---|---|
| `wlcs` headless conformance | COMP-15 §1 | harness and the blocking `conformance` job exist; the suite runs green off-CI (755 passed / 308 skipped / 0 failed at `ulimit -n 1024`, against 85 skip entries; of the skips only foreign-toplevel's 30 are a real gap, the rest being dead protocols and wlcs self-tests); awaiting a green run on CI’s llvmpipe runner |
