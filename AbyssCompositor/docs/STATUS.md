...
| 9c | `headless` backend (COMP-01 §10) | **partially landed** | `backend/headless.rs` runs: EGL device (hardware render node, else Mesa software), offscreen GLES target, synthetic 60 Hz frame clock, no KMS and no host display server. `abyss --backend headless [--size WxH]` serves a real socket and composites for real, so redaction and pixel comparisons have something to assert on. `cargo check -p abyss --no-default-features --features headless` is green, so the feature is self-contained. **Missing:** the `wlcs` harness and the blocking CI job — no `tests/wlcs/` exists and `gate.yml` has no headless job — so the milestone's exit gate is not met. |
...
|---|---|---|
| `wlcs` headless conformance | COMP-15 §1 | the `headless` backend exists (9c); the `wlcs` harness and its blocking CI job do not |
